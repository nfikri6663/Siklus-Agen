function scr_current_action(){
    ...
    for (var i = 0; i < _max_rows; i++){
        ...
        _panel_y2 += _line_height*(_max_rows > 8 ? 0.7 : 1)
    }
...
function draw_action_text(_x, _y, _actions, _index, _highlight){
    if _index < array_length(_actions){
        ...
        if _enum >= 0 && _enum < array_length(action_names){
            draw_set_color(is_simulating || game_phase == "auto_learning" ? (_highlight ? c_lime : c_white) : c_white)
            var _scale = array_length(_actions) > 8 ? 0.8 : 1
            draw_text_transformed(_x, _y, action_names[_enum],_scale,_scale,0)
        }
    }
}