function update_q_table(_state, _action, _reward, _next_state){
    if _action < 0 || _action >= ACTIONS._count{// Validasi aksi
        show_debug_message("ERROR: Aksi tidak valid saat update Q-Table: " + string(_action))
        return undefined
    }
    
    if is_undefined(q_table[? _state]) q_table[? _state] = array_create(ACTIONS._count, 0)
    if is_undefined(q_table[? _next_state]) q_table[? _next_state] = array_create(ACTIONS._count, 0)
    
    // Ambil nilai Q lama
    var _old_q_values = q_table[? _state]
    var _old_q_value = _old_q_values[_action]
    
    // Cari nilai Q terbaik untuk next_state (State Selanjutnta)
    var _next_q_values = q_table[? _next_state]
    var _max_next_q = _next_q_values[0]
    for (var i = 1; i < array_length(_next_q_values); i++){
        if (_next_q_values[i] > _max_next_q) _max_next_q = _next_q_values[i]
    }
