...
    draw_text(_x_title + (_table_end_x - _x_title)/2, _report_y, $"== LAPORAN HARI KE - {max(0, days_passed)} ==")
...
        draw_set_halign(fa_center)
        draw_set_color(c_white)
        draw_text(_x_reward, _yy, $"{max(0, result.reward)} / {min(0, result.reward)}")
...
        var _entry = current_day_log[i]
        ...
        if variable_struct_exists(_entry, "time_of_day") && _entry.time_of_day != _last_drawn_time{
            ...
            var _full_state_text = _entry.state_key_at_the_time
            ...
            var _header_text = $"--- {string_upper(_entry.time_of_day)}: {string_upper(_clean_state_text)} ---"
