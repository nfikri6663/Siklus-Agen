// Mengembalikan sebuah struct berisi {name: "Aksi", reason: "Alasannya"}
function get_recommendation_details(_state_key, _valid_actions) {
    // Kasus 1: Tidak ada aksi
    ...
    if (is_undefined(_q_values)) {
        ...
        return { name: action_names[_valid_actions[0]], reason: "mencoba hal baru", result_struct: _potential_result, q_value: 0 };
    }
    
    // Kasus 3: Logika Normal
    ...
    // Lakukan dry run untuk mendapatkan detail hasil
    var _potential_result = perform_action_and_get_reward(_best_action_enum, true);
    
    // Tentukan alasan berdasarkan kondisi agen
    // --- Langkah B: Tentukan alasan berdasarkan aksi terbaik dan kondisi agen ---
    var _reason = "";
    ...
    // Kembalikan struct lengkap untuk kasus normal
    return { 
