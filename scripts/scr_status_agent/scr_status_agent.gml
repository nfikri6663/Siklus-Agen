function scr_status_agent(){
    _panel_x = 10;
    _panel_y = 10;
    _line_height = 24; // Beri sedikit ruang lebih untuk bar
    var _panel_width = 220;
    _col_label_x = _panel_x + 10;
    _col_value_x = _panel_x + 120;
    _bar_width = 90;
    
    // Gambar judul dan latar belakang panel
    ...
    var _draw_stat_row = function(label, value, max_value, show_bar=false, invert_color=false) {
        // Gambar Label (misal: "Health")
        draw_set_color(c_white);
        draw_set_valign(fa_middle);
        draw_text(_col_label_x, _panel_y, label + ":");
        
        // Jika ada bar, gambar barnya
        if (show_bar) {
            ...
            // Warna bar (bisa dibalik untuk stress, di mana rendah itu bagus)
            ...
            var c = c_white;
            draw_set_halign(fa_center);
            draw_text_transformed_color(_bar_x + _bar_width/2, _panel_y+1, string(value),0.8,0.8,0,c,c,c,c,1);
            draw_set_halign(fa_left);
        } else {
            // Jika tidak ada bar, gambar teks nilainya
            draw_set_color(c_white);
            draw_text(_col_value_x, _panel_y, string(value));
        }
        
        _panel_y += _line_height; // Pindah ke baris berikutnya
    }
    
    // --- Panggil fungsi pembantu untuk setiap stat ---
    ...
    _draw_stat_row("Stress", string_format(stress, 1, 0), 100, true, true); // Invert warna untuk stress
    _draw_stat_row("Social", string_format(social, 1, 0), 100, true);
    
    // Untuk stat tanpa bar
