if game_phase == "auto_learning"{
    if (step_training[step_num] % 3) == 0 current_day_log = []
    run_ai_cycle()
    if step_training[step_num] > (_training_cycles-1){
        if step_num < array_length(step_training)-1{
            audio_play_sound(sn_one_cycle_turn,1,false)
            step_num ++
        }
        if step_num == array_length(step_training)-1{
            audio_play_sound(sn_training_finished,1,false)
            alarm[1] = 1
        }
    }
    alarm[0] = 1;
...
var _sick_chance = 4
if age > 60 _sick_chance = 20
if irandom(99) < _sick_chance && !is_sick{
